      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            animate={{ width: tooltipWidth, height: "auto" }}
            exit={{
              width: 0,
              height: 0,
            ...
            transition={{
              width: { type: "spring", visualDuration: 0.4, bounce: 0.34 },
              height: { type: "spring", visualDuration: 0.4, bounce: 0.34 },
            }}
            className="pointer-events-none absolute z-50 w-[300px] max-w-none overflow-hidden rounded-md border border-transparent bg-white shadow-sm ring-1 shadow-black/5 ring-black/5 dark:bg-neutral-900 dark:shadow-white/10 dark:ring-white/5"
            style={{
              top: position.y,
              left: position.x,
            }}
          >
            <motion.div
              ref={contentRef}
              initial={{ filter: "blur(5px)", scale: 1.25 }}
              animate={{ filter: "blur(0px)", scale: 1 }}
              transition={{
                filter: { duration: 0.28, ease: [0.16, 1, 0.3, 1] },
                scale: { type: "spring", visualDuration: 0.36, bounce: 0.2 },
              }}
              style={{ transformOrigin: "top left" }}
              className="w-[300px] p-2 text-sm text-neutral-600 md:p-4 dark:text-neutral-400"
            >
              {content}
            </motion.div>
          </motion.div>
